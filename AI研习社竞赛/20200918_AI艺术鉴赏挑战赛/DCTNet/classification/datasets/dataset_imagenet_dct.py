...
class DatasetFolderDCT(VisionDataset):
    def __init__(self,
        ...
        samples, cls_num = make_dataset(self.root, class_to_idx, extensions,
                                        is_valid_file)
        if len(samples) == 0:
    ...
    def __getitem__(self, index):
        ...
        if self.backend == 'opencv':
            ...
            sample = self.loader(path, backend='pil', colorSpace='BGR')
...
if __name__ == '__main__':
    ...
    print('The mean of dct_y is: {}'.format(
        np.asarray(dct_y_mean_total).mean(axis=0)))
    print('The std of dct_y is: {}'.format(
        np.asarray(dct_y_std_total).mean(axis=0)))
