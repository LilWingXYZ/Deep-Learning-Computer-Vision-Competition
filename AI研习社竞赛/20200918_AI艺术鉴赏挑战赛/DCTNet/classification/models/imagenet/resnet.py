...
from gate import GateModule192, GateModule

__all__ = [
    'ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152',
    'resnext50_32x4d', 'resnext101_32x8d', 'ResNetDCT_Upscaled_Static',
    'ResNet50DCT'
]
...
class ResNet50DCT(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.deconv1 = nn.ConvTranspose2d(in_channels=in_ch,
                                          out_channels=out_ch,
                                          kernel_size=4,
                                          stride=2,
                                          padding=1)
        self.deconv2 = nn.ConvTranspose2d(in_channels=in_ch,
                                          out_channels=out_ch,
                                          kernel_size=4,
                                          stride=2,
                                          padding=1)
...
class ResNet50DCT_Upscaled(nn.Module):
    def __init__(self):
        ...
        self.upconv_y = nn.Conv2d(in_channels=64,
                                  out_channels=22 * upscale_factor_y *
                                  upscale_factor_y,
                                  kernel_size=3,
                                  stride=1,
                                  padding=1,
                                  bias=False)
        self.upconv_cb = nn.Conv2d(in_channels=64,
                                   out_channels=21 * upscale_factor_cb *
                                   upscale_factor_cb,
                                   kernel_size=3,
                                   stride=1,
                                   padding=1,
                                   bias=False)
        self.upconv_cr = nn.Conv2d(in_channels=64,
                                   out_channels=21 * upscale_factor_cr *
                                   upscale_factor_cr,
                                   kernel_size=3,
                                   stride=1,
                                   padding=1,
    ...
    def init_weights(self):
        ...
        for name, m in self.named_modules():
            if any(s in name for s in [
                    'upconv_y', 'upconv_cb', 'upconv_cr', 'bn_y', 'bn_cb',
                    'bn_cr'
            ]):
                if isinstance(m, nn.Conv2d):
                    kaiming_init(m)
    ...
    def forward(self, dct_y, dct_cb, dct_cr):
        ...
        dct_cb = self.relu(
            self.bn_cb(self.pixelshuffle_cb(self.upconv_cb(dct_cb))))
        dct_cr = self.relu(
            self.bn_cr(self.pixelshuffle_cr(self.upconv_cr(dct_cr))))
        x = torch.cat((dct_y, dct_cb, dct_cr), dim=1)
...
class ResNetDCT_Upscaled_Static(nn.Module):
    def __init__(self, channels=0, pretrained=True, input_gate=False):
        ...
        self.model = nn.Sequential(*list(model.children())[4:-1])
        #self.relu = nn.ReLU(inplace=True)
        self.fc1 = list(model.children())[-1]
        if channels == 0 or channels == 192:
            out_ch = self.model[0][0].conv1.out_channels
            self.model[0][0].conv1 = nn.Conv2d(channels,
                                               out_ch,
                                               kernel_size=1,
                                               stride=1,
                                               bias=False)
            ...
            self.model[0][0].downsample[0] = nn.Conv2d(channels,
                                                       out_ch,
                                                       kernel_size=1,
                                                       stride=1,
                                                       bias=False)
            ...
            out_ch = self.model[0][0].conv1.out_channels
            # temp_layer = conv3x3(channels, out_ch, 1)
            temp_layer = nn.Conv2d(channels,
                                   out_ch,
                                   kernel_size=3,
                                   stride=1,
                                   bias=False)
            temp_layer.weight.data = self.model[0][
                0].conv1.weight.data[:, :channels]
            self.model[0][0].conv1 = temp_layer
            ...
            temp_layer = nn.Conv2d(channels,
                                   ...
                                   stride=1,
                                   bias=False)
            temp_layer.weight.data = self.model[0][0].downsample[
                0].weight.data[:, :channels]
            self.model[0][0].downsample[0] = temp_layer
